...
class GavelPolicy(object):
    ...
    def populate_valid_configs(self, cluster_num_nodes, cluster_num_gpus, cluster_max_physical_nnodes=None):
        ...
        for cname in self._cluster_name:
            self._cluster_spec[cname] = cluster_num_nodes[cname] * \
                cluster_num_gpus[cname]
        ...
        for worker in worker_ids:
            for cid in range(len(self._cluster_name)):
                if cname == self._cluster_name[cid]:
                    ...
                    res.append(math.floor(
                        idx / self._num_gpus_per_server[cname]))
    ...
    def get_allocation_state(self):
        ...
        state['prestart_overhead'] = {
            job_id: self._time_per_iteration -
            (job.submission_time % self._time_per_iteration) + job.rescale_time
            for job_id, job in self._jobs.items()
        }
        ...
        state['throughputs'] = {job_id: {cname: self.predict_throughput(
            job, cname, job_id) for cname in self._cluster_name} for job_id, job in self._jobs.items()}
        # state['throughputs'] = {}
        # for job_id, job in self._jobs.items():
        #     ths = {}
        #     for cname in self._cluster_name:
        ...
        for cname in self._cluster_name:
            ...
            for job_id in self._jobs:
                ...
                if self._allocation[job_id][cname] == 0.0:
                    ...
                    new_priority = self._allocation[job_id][cname] / \
                        fractions[cname][job_id]
    ...
    def schedule_jobs_on_workers(self):
        ...
        print("### selected jobs:", sum(
            [len(v) for _, v in scheduled_jobs.items()]), "/", len(self._jobs))
    ...
    def optimize(self, jobs, nodes, prev_allocations):
        ...
        print("########################## Start ##################################")
        print(
            f"Time: {self._current_time} Round: {self._current_time/self._time_per_iteration}")
    ...
    def predict_throughput(self, job, cname, job_id):
        ...
        num_gpu_per_node = int(
            self.configs[cname][1][-1] / self.configs[cname][0][-1])
        ...
        while sum(placement) < job.scale_factor:
            placement = (*placement, min(job.scale_factor -
                         sum(placement), num_gpu_per_node))

        local_bsz = math.ceil(job.target_batch_size / job.scale_factor - 1e-8)
        max_local_bsz = job.applications[cname].get_max_local_bsz(placement)
        accum_steps = math.ceil(
            local_bsz / max_local_bsz - 1e-8) - 1
        ...
        print(
            f"PREDICT_XPUT:: {job_id}, {cname} x {placement}, atomic_bsz: {atomic_bsz} accum_steps: {accum_steps}-> {xput}")
        assert xput > 0, f"{job_id}, {cname} x {placement}, atomic_bsz: {atomic_bsz} accum_steps: {accum_steps} "
        xput = xput * job.slowdown_factor
        return xput
