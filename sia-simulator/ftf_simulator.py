...
class FTFCluster(object):
    ...
    def __init__(self, workload, policy, num_nodes, ngpus_per_node, job_max_replicas):
        ...
        if isinstance(policy, SiaPolicy) or isinstance(policy, PolluxPolicyUnaware):
            ...
            assert self.jobs[0].target_num_replicas == list(workload.itertuples())[0].num_replicas
            ...
            for job in self.jobs:
                if job.submission_time <= self.current_time and job.completion_time is None:
                    ...
                    if isinstance(self.policy, SiaPolicy) or isinstance(self.policy, GavelPolicy):
                        ...
                        assert len(alloc) == job.target_num_replicas, f'{job.name} target_num_replicas: {job.target_num_replicas} alloc: {alloc}'
                        # change in resources
    ...
    def simulate_jct(self, interval):
        assert len(self.jobs) == 1
        assert len(self.clusters) == 1
        job = self.jobs[0]
        cname = self.clusters[0]
        app = job.applications[cname]
        total_steps = app.get_iteration(job.target_batch_size, job.completion_epoch)
        iter_time = self.predict_iter_time(job, cname)
        print({job.name: total_steps * iter_time})
        rescale_time = job.applications[cname].rescale_time
        slowdown_factor = job.job.calibration_factor
        return {job.name: interval - job.submission_time % interval + rescale_time + ((total_steps * iter_time) / slowdown_factor)}


    def predict_iter_time(self, job, cname):
        placement = ()
        num_gpu_per_node = self.num_gpus[cname]
        # no enough gpus in this cluster
        if job.target_num_replicas > self.num_gpus[cname] * self.num_nodes[cname]:
            assert False
            return 1e-1
        while sum(placement) < job.target_num_replicas:
            placement = (*placement, min(job.target_num_replicas - sum(placement), num_gpu_per_node))

        local_bsz = math.ceil(job.target_batch_size / job.target_num_replicas - 1e-8)
        accum_steps = math.ceil(local_bsz / job.applications[cname].max_local_bsz - 1e-8) - 1
        if job.target_num_replicas == 1:
            accum_steps = max(1, accum_steps)
        atomic_bsz = math.ceil(local_bsz / (accum_steps + 1) - 1e-8)
        count = job.target_num_replicas * (accum_steps + 1)
...
def simulate_single_job(args, job_df, max_num_replicas, cluster_name, cluster_nnodes, cluster_ngpus_per_node):
    ...
    if job_max_nodes > cluster_max_physical_nnodes[cluster_name]:
        print(f"BAD CONFIG FOR GAVEL")
        return {job_df['name'].values[0]: 0}
    ...
    # if args.policy == "gavel" or args.policy == "shockwave":
    #    return simulator.simulate_jct(args.interval)
...
def generate_workload_jobs(workload_file, args, cluster_nnodes, cluster_ngpus_per_node):
    ...
    num_replicas_in_file = {}
    for i, row in workload.iterrows():
        num_replicas_in_file[row['name']] = row.num_replicas
        print(f"CORRECTING NUM REPLICAS: {row['name']} -> {num_replicas_in_file[row['name']]}")
    ...
    for k, v in num_replicas.items():
        ...
        if POLICY == "gavel":
            lower_max_replicas[k] = 0
            upper_max_replicas[k] = num_replicas_in_file[k]
        cluster_args = {}
...
def combine_cluster_jcts(cluster_jcts, cluster_nnodes, cluster_ngpus_per_node):
    ...
    for cluster_name in cluster_nnodes.keys():
        ...
        if cluster_jcts[cluster_name] == 0:
            print(f"DROPPING {cluster_name} for jct=0")
            continue
...
def simulate_dir(args, cluster_nnodes, cluster_ngpus_per_node):
    ...
    #     print(args_)
    #     input('continue')
    #     jct_dict = simulate_single_job(*args_)
    #     jcts_list.append(jct_dict)
    #     print(f'!'*40, f'{i} / {len(args_list)} finihsed, jct: {jct_dict}')

    with multiprocessing.Pool(processes=args.num_procs) as pool:
        jcts_list = pool.starmap(simulate_single_job, args_list, chunksize=1)
    ...
    def interpolate_jct_gavel(jobname, lower_idx, upper_idx, num_replicas, exec_replicas):
        print(f"{jobname}, {num_replicas} -> {exec_replicas}")
        lower_jct_dict, upper_jct_dict = jcts_list[lower_idx], jcts_list[upper_idx]
        lower_jct, upper_jct = lower_jct_dict[jobname], upper_jct_dict[jobname]
        if exec_replicas > num_replicas:
            scale = (exec_replicas * 1.0 / num_replicas)
            jct = upper_jct * scale
        else:
            jct = upper_jct
        return jct
    ...
    for workload_name in dict_to_arglist_idxmap.keys():
        ...
        for jobname, job_cluster_idxs_dict in dict_to_arglist_idxmap[workload_name].items():
            ...
            for cluster_name, cluster_idxs in job_cluster_idxs_dict.items():
                ...
                lower_args, upper_args, num_replicas = args_dict[workload_name][jobname][cluster_name]
                if POLICY != "gavel":
                    cluster_jct = interpolate_jct(jobname=jobname, lower_idx=lower_idx, upper_idx=upper_idx, num_replicas=num_replicas)
                else:
                    cluster_jct = interpolate_jct_gavel(jobname=jobname, lower_idx=lower_idx, upper_idx=upper_idx, num_replicas=num_replicas, exec_replicas=upper_args[2])
